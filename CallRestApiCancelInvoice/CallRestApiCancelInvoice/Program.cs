...
try
{
    ...
    var result = await http.PostAsJsonAsync(requestUri: $"{Api}/api/CancelInvoices", invoices);
    ...
    while (result.StatusCode == HttpStatusCode.OK)
    {
		...
		result = await http.PostAsJsonAsync(requestUri: $"{Api}/api/CancelInvoices", invoices);
    }
